 ...
 *      - disable SPI and unconfigure Slave Select pin 
 *        when mode ARM_SPI_MODE_INACTIVE is activated
 *      - activate internal pull-up/down on SCK pin according to 
 *        clock/frame format setting
 *      - set Slave Select pin to inactive state after 
 *        ARM_SPI_SS_MASTER_SW is activated
...
#define ARM_SPI_DRV_VERSION ARM_DRIVER_VERSION_MAJOR_MINOR(2,16)
...
static int32_t SPI_Transfer (const void *data_out, void *data_in, uint32_t num, const SPI_RESOURCES *spi) {
  ...
  if ((spi->rx_dma != NULL) || (spi->tx_dma != NULL)) {
    ...
    if (spi->rx_dma != NULL) {
      // Disable Rx Not Empty, used only until Rx is started with DMA
      // to allow data lost detection
...
static int32_t SPI_Control (uint32_t control, uint32_t arg, const SPI_RESOURCES *spi) {
  ...
  if ((control & ARM_SPI_CONTROL_Msk) == ARM_SPI_ABORT_TRANSFER) {
    ...
    SPI_PeripheralReset (spi->reg);
    spi->reg->CR2 = cr2 | SPI_CR2_RXNEIE;
    spi->reg->CR1 = cr1 & (~SPI_CR1_SPE);

    memset((void *)spi->xfer, 0, sizeof(SPI_TRANSFER_INFO));
    spi->info->status.busy = 0U;

    // Restore CR1 settings
    spi->reg->CR1 = cr1;
  ...
  switch (control & ARM_SPI_FRAME_FORMAT_Msk) {
    case ARM_SPI_CPOL0_CPHA0:
      // Configure SPI SCK pin with pull-down enabled
      GPIO_InitStruct.Pin       = spi->io.sck->pin;
      GPIO_InitStruct.Mode      = GPIO_MODE_AF_PP;
      GPIO_InitStruct.Speed     = spi->io.sck->speed;
      GPIO_InitStruct.Alternate = spi->io.sck->af;
    ...
    case ARM_SPI_CPOL1_CPHA0:
      cr1 |= SPI_CR1_CPOL;
      // Configure SPI SCK pin with pull-up enabled
      GPIO_InitStruct.Pin       = spi->io.sck->pin;
      GPIO_InitStruct.Mode      = GPIO_MODE_AF_PP;
      GPIO_InitStruct.Speed     = spi->io.sck->speed;
      GPIO_InitStruct.Alternate = spi->io.sck->af;
  ...
  // Configure registers
  spi->reg->CR1 &= ~SPI_CR1_SPE;
  spi->reg->CR2  = cr2 | SPI_CR2_ERRIE | SPI_CR2_RXNEIE;
  spi->reg->CR1  = cr1;
