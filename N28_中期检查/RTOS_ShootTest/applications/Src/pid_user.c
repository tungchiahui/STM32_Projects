...
float motor_speed_3508_pid_1[3] = {10, 0.1, 0};//底盘3508参数
float motor_speed_3508_pid_2[3] = {15, 0.1, 0};//底盘3508参数
...
void PID_devices_Init(void)
{
	...
	PID_init(&pid_v_1[0], PID_POSITION, motor_speed_3508_pid_1, 10000, 6000);
	PID_init(&pid_v_1[1], PID_POSITION, motor_speed_3508_pid_2, 10000, 6000);
	for(int i=0;i<4;i++)
	{
	...
	for(int i=4;i<8;i++)
	{		
    PID_init(&pid_v_1[i], PID_POSITION, motor_speed_2006_pid, 10000, 6000);
		PID_init(&pid_pos_1[i], PID_POSITION, motor_position_2006_pid, 400, 300);
