 ...
 *     - clear transfer information when powered off
 *    Corrected Send function to:
 *     - not start reception in Synchronous Master mode
 *     - clear Transmission Complete status before starting Tx transfer (in DMA mode)
 ...
 *     - for abort Send/Transfer wait for already loaded frame to be sent
 ...
 *     - enable parity error detection if parity usage is configured
 ...
 *     - corrected RX Timeout handling
 *     - corrected break signal detection
 *     - commented that CTS event detection cannot work without flow control
...
#define ARM_USART_DRV_VERSION ARM_DRIVER_VERSION_MAJOR_MINOR(2,14)
...
static int32_t USART_Receive (      void            *data,
...
#endif
  {
    ...
    usart->reg->CR1 |= (USART_CR1_IDLEIE | USART_CR1_RXNEIE | USART_CR1_TE);
  }  
...
static int32_t USART_Control (      uint32_t          control,
    ...
    case ARM_USART_ABORT_SEND:
      ...
      if (usart->xfer->send_active != 0U) {
        ...
        for (i = 0U; i < 100000U; i++) {
          if ((usart->reg->SR & USART_SR_TC) != 0U) {
            break;
          }
        }
      }
  ...
  switch (control & ARM_USART_PARITY_Msk) {
    ...
    case ARM_USART_PARITY_EVEN:   cr1 |=  USART_CR1_PCE  | 
                                          USART_CR1_PEIE ;   break;
...
void USART_IRQHandler (const USART_RESOURCES *usart) {
  ...
  // IDLE line
  if ((sr & USART_SR_IDLE & usart->reg->CR1) && ((sr & USART_SR_RXNE & usart->reg->CR1) == 0U)) {
