...
#define ARM_MCI_DRV_VERSION ARM_DRIVER_VERSION_MAJOR_MINOR(2,10)  /* driver version */
    ...
    case ARM_POWER_OFF:
      ...
      __HAL_RCC_SDIO_RELEASE_RESET();
        ...
        if (hdma_sdio_rx.Instance != NULL) {
          /* Disable rx stream interrupt in NVIC */
          HAL_NVIC_DisableIRQ (SDIO_RX_DMA_IRQn);

          /* Abort potential transfers */
          (void)HAL_DMA_Abort(&hdma_sdio_rx);
        ...
        if (hdma_sdio_tx.Instance != NULL) {
          /* Disable tx stream interrupt in NVIC */
          HAL_NVIC_DisableIRQ (SDIO_TX_DMA_IRQn);

          /* Abort potential transfers */
          (void)HAL_DMA_Abort(&hdma_sdio_tx);
