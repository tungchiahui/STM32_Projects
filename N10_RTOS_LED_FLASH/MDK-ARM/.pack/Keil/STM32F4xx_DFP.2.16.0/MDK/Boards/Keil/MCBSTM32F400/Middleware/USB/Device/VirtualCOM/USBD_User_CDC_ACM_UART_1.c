 ...
 *------------------------------------------------------------------------------
 * Name:    USBD_User_CDC_ACM_UART_1.c
 ...
 *
 * USBD_User_CDC_ACM_UART_1.c implements the application specific
 ...
 *     the UART is sent over USB using the CDC1_ACM_UART_to_USB_Thread thread.
 *   USB -> UART:
 *     While the UART transmit is not busy, data transmission on the UART is
 *     started in the USBD_CDC1_ACM_DataReceived callback as soon as data is
...
#define  UART_PORT              3       // UART Port number
...
extern   void                   CDC1_ACM_UART_to_USB_Thread (void const *arg);
#endif
...
static void UART_Callback (uint32_t event) {
  ...
  if (event & ARM_USART_EVENT_SEND_COMPLETE) {
    // USB -> UART
    cnt = USBD_CDC_ACM_ReadData(1U, uart_tx_buf, UART_BUFFER_SIZE);
    if (cnt > 0) {
  ...
  for (;;) {
    ...
    if (ptrUART->GetStatus().rx_busy != 0U) {
      ...
      if (cnt > 0) {
        ...
        cnt = USBD_CDC_ACM_WriteData(1U, (uart_rx_buf + ((uint32_t)usb_tx_cnt & (UART_BUFFER_SIZE - 1))), cnt);
        if (cnt > 0) {
          usb_tx_cnt += cnt;
        }
      }
...
#else
extern const osThreadDef_t os_thread_def_CDC1_ACM_UART_to_USB_Thread;
osThreadDef (CDC1_ACM_UART_to_USB_Thread, osPriorityNormal, 1U, 0U);
#endif
...
#else
  cdc_acm_bridge_tid = osThreadCreate (osThread (CDC1_ACM_UART_to_USB_Thread), NULL);
#endif
}
...
bool USBD_CDC1_ACM_GetLineCoding (CDC_LINE_CODING *line_coding) {
  ...
  // Load settings from ones stored on USBD_CDC1_ACM_SetLineCoding callback
