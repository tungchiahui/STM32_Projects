 ...
 *------------------------------------------------------------------------------
 * Name:    USBD_User_CDC_ACM_RNDIS_ETH_0.c
 ...
 *
 * USBD_User_CDC_ACM_RNDIS_ETH_0.c implements the application specific
 ...
 * to the USB Host.
 *
 * The implementation depends on the configuration file USBD_Config_CDC_0.h.
 *
 */
...
static uint32_t           packet_in [(USBD_CDC0_ACM_SEND_BUF_SIZE   +3)/4];
static uint32_t           packet_out[(USBD_CDC0_ACM_RECEIVE_BUF_SIZE+3)/4];
...
bool USBD_CDC0_ACM_SendEncapsulatedCommand (const uint8_t *buf, uint16_t len) {
  ...
  switch (msg_type) {                       // MessageType
    case REMOTE_NDIS_INITIALIZE_MSG:
      ...
      ptr_init_cmplt->MaxTransferSize        = USBD_CDC0_ACM_RECEIVE_BUF_SIZE;
    ...
    case REMOTE_NDIS_QUERY_MSG:
      ...
      switch (ptr_query_msg->Oid) {             // Handle OID
        ...
        case OID_GEN_TRANSMIT_BLOCK_SIZE:
          ptr_query_cmplt->InformationBufferLength = 4U;
          ptr_query_cmplt->OIDInputBuffer[0]       = USBD_CDC0_ACM_SEND_BUF_SIZE;
          break;
        case OID_GEN_RECEIVE_BLOCK_SIZE:
          ptr_query_cmplt->InformationBufferLength = 4U;
          ptr_query_cmplt->OIDInputBuffer[0]       = USBD_CDC0_ACM_RECEIVE_BUF_SIZE;
          break;
  ...
  if (get_encapsulated_response_len != 0U) {
    ...
    (void)USBD_CDC_ACM_Notify_ResponseAvailable (0);
  }
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if ((link_state  == ARM_ETH_LINK_UP)        &&
        ...
        do {
          usb_cdc_acm_status = USBD_CDC_ACM_WriteData (0, (const uint8_t *)ptr_packet_msg, (int32_t)ptr_packet_msg->MessageLength);
...
#endif
    for (;;) {
      usb_cdc_acm_status = USBD_CDC_ACM_ReadData (0, (uint8_t *)ptr_packet_msg, USBD_CDC0_ACM_RECEIVE_BUF_SIZE);
