 ...
 *----------------------------------------------------------------------------*/
/*
 * USBD_User_CustomClass_1.c is a code template for the Custom Class 1 
 ...
 * Class configuration in USBD_Config_CustomClass_1.h file.
 */
...
void USBD_CustomClass1_EndpointStart (uint8_t ep_addr) {
  ...
  if (USBD_GetState(1U).speed == USB_SPEED_HIGH) {
  ...
  if (ep_addr == USB_ENDPOINT_OUT(1)) {
    USBD_EndpointRead(1U, USB_ENDPOINT_OUT(1), bulk_out_buf, bulk_max_packet_size);
  }
}
...
void USBD_CustomClass1_Endpoint1_Event  (uint32_t event) {
  ...
  if (event & ARM_USBD_EVENT_OUT) {     // OUT event
    data_len_received = USBD_EndpointReadGetResult(1U, USB_ENDPOINT_OUT(1));
    data_received     = true;
  }
  if ((data_received) && (!send_active)) {
    ...
    send_active = true;
    USBD_EndpointWrite(1U, USB_ENDPOINT_IN(1) , bulk_in_buf, data_len_received);
