...
void Update_x(float new_x)
{
	...
	for(i=0;i<8;i++)
	{
		HAL_UART_Transmit(&huart1,(uint8_t *)&update_x,1,HAL_MAX_DELAY);
...
//		USART_SendData(USART6, update_y);		
		HAL_UART_Transmit(&huart1,(uint8_t *)&update_y,1,HAL_MAX_DELAY);
	}
}
...
//		USART_SendData(USART6, update_yaw);		
		HAL_UART_Transmit(&huart1,(uint8_t *)&update_yaw,1,HAL_MAX_DELAY);
	}
}
