...
{
	int16_t drive_motor_current[4] = {0};
	int16_t swerve_motor_current[4] = {0};
	fp32 output_angle[4];

	for(int i = 0;i < 4;i++)
	{
		output_angle[i] = chassisOri.swerve_motor.wheel_total_angle[i] / 360.0f * M2006_REDU_RATIO * 8192.0f * GEAR_REDU_RATIO;
		swerve_motor_current[i] = pid_controller.can_motor.CAN1_VP_Dual_Loop_Realize(output_angle[i],i+4);
		drive_motor_current[i] = pid_controller.can_motor.CAN1_Velocity_Realize(chassisOri.drive_motor.wheel_rpm[i],i);
	}
	
	can_bus.cmd.CAN1_Front(drive_motor_current[0],drive_motor_current[1],drive_motor_current[2],swerve_motor_current[3]);
	can_bus.cmd.CAN1_Behind(swerve_motor_current[0],swerve_motor_current[1],swerve_motor_current[2],swerve_motor_current[3]);
}
