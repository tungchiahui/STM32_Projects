...
extern int16_t opencv_tracking_offset;


pid_type_def pid_v_1[8],pid_pos_1[8];
pid_type_def pid_v_2[8],pid_pos_2[8];
pid_type_def pid_offset_drive;
pid_type_def pid_offset_helm;
...
fp32 motor_offset_drive_2006_pid[3] = {1,0,0};
fp32 motor_offset_helm_2006_pid[3] = {0.1,0,0};

// fp32 motor_speed_2006_catch[3] = {9,0.1,0.3};  //爪子
// fp32 motor_position_2006_catch[3] = {0.2,0.005,0};  //爪子
...
void PID_devices_Init(void)
{
	...
	for(int i=3;i<6;i++)
	{		
    PID_init(&pid_v_1[i], PID_POSITION, motor_speed_2006_pid, 9000, 6000);
		PID_init(&pid_pos_1[i], PID_POSITION, motor_position_2006_pid, 8000, 2000);
	}
	...
	// 	PID_init(&pid_pos_2[i], PID_POSITION, motor_position_2006_catch, 4000, 300);
	// }
	
	// //推环PID
	// PID_init(&pid_v_2[6], PID_POSITION, motor_speed_2006_push, 10000, 6000);
	// PID_init(&pid_pos_2[6], PID_POSITION, motor_position_2006_push, 3500, 0);
	
	//全局定位PID
	// PID_init(&pid_yaw,PID_POSITION,motor_yaw_pid,3000,1500);
	// PID_init(&pid_pos_x,PID_POSITION,motor_pos_x_pid,1500,1500);
	// PID_init(&pid_pos_y,PID_POSITION,motor_pos_y_pid,1500,1500);
}
...
fp32 PID_opencv_tracking_offset_drive(fp32 set_offset)
{
	PID_calc(&pid_offset_drive,opencv_tracking_offset,set_offset);
	return pid_offset_drive.out;
}

fp32 PID_opencv_tracking_offset_helm(fp32 set_offset)
{
	PID_calc(&pid_offset_helm,opencv_tracking_offset,set_offset);
	return pid_offset_helm.out;
}
