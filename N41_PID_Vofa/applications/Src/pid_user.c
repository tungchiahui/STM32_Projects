...
float motor_speed_3508_pid[3] = {15, 0, 0};//3508参数
float motor_position_3508_pid[3] = {0.2, 0, 1};
float motor_speed_2006_pid[3] = {11, 0.01, 0};//2006参数
float motor_position_2006_pid[3] = {0.2, 0.005, 0};
fp32 motor_speed_2006_catch[3] = {9,0.1,0.3};  //爪子
fp32 motor_position_2006_catch[3] = {0.2,0.005,0};  //爪子
fp32 motor_speed_2006_push[3] = {15,0.001,0};   //推环
fp32 motor_position_2006_push[3] = {0.25,0.02,0.01};   //推环
...
void PID_devices_Init(void)
{
    ...
    PID_init(&pid_v_1[i], PID_POSITION, motor_speed_3508_pid, 10000, 6000);
		...
		PID_init(&pid_pos_2[i], PID_POSITION, motor_position_2006_pid, 3500, 300);
	}
	
	for(int i=4;i<8;i++)
	{		
    PID_init(&pid_v_1[i], PID_POSITION, motor_speed_3508_pid, 10000, 6000);
		PID_init(&pid_pos_1[i], PID_POSITION, motor_position_3508_pid, 3500, 300);
	...
	PID_init(&pid_pos_2[6], PID_POSITION, motor_position_2006_push, 3500, 450);
}
